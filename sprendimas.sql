...
SELECT
	`prekes`.`preke`
	, GROUP_CONCAT (`prekes_tiekejai`.`barkodas`) AS `visi_barkodai`
	, GROUP_CONCAT (`prekes_tiekejai`.`nuo_datos`)
	, COUNT (`prekes_tiekejai`.`barkodas`) AS `barkodas`
FROM
	`prekes`
LEFT JOIN `prekes_tiekejai` ON (
	`prekes`.`id` = `prekes_tiekejai`.`id_prekes`
	)
GROUP BY `prekes`.`id`
HAVING 
	`barkodai` > 1
